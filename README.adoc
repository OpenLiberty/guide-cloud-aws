...
- Lastly, in this guide we will use the `eksctl` CLI tool for provisioning your EKS cluster. 
...
When tagging and pushing your images, remember to subtitute `[name-repository-uri]` and 
...
Make a request to the `name` microservice substituting in `ip-address`.
...
Make a request to the `ping` microservice substituting in `ip-address`.
...
Since you built a new image, it will have to be pushed to the `awsguide/name` repository
