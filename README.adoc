...
and distribute your container images. Note, because EKS is not free, there is a small
...
* *eksctl:* In this guide, you will use the `eksctl` Command Line Interface (CLI) tool for provisioning your EKS cluster. 
...
of the `AWS Access Key ID` and `AWS Secret Access Key`. After the AWS CLI is installed, it 
must be configured by running the AWS configure command. 
...
The registry that you are using is called Amazon Elastic Container Registry (ECR).
...
The `get-login` command returns a `[password_string]`; take a note of this `[password_string]`. 
Next, running the following will return the `[aws_account_id]` needed to authenticate your Docker client. 
...
Replace the `[password_string]`, `[aws_account_id]` and the `[region]` your account is configured 
under in the following `docker login` command, that is used to authenticate your Docker client. 
