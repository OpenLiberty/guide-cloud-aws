...
:page-search-keywords: ['Amazon Web Services', 'EKS', 'ECR', 'ec2', 'Kubernetes', 'eksctl', 'container registry']
...
documentation to download the latest version of Python. After Python is installed, install the AWS CLI by following the instructions in the official
...
| [hotspot=sysKubeService]`system.ip` | Name of the {kube} Service wrapping the `system` pods, `system-service` by default.
| [hotspot=invKubeService]`inventory.ip` | Name of the {kube} Service wrapping the `inventory` pods, `inventory-service` by default.
| [hotspot=sysHttpPort]`system.http.port`    | The HTTP port for the {kube} Service `system-service`, 9080 by default.
| [hotspot=invHttpPort]`inventory.http.port`    | The HTTP port of the {kube} Service `inventory-service`, 9081 by default.
|===
