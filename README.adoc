...
simply returns the JVM systems properties of the container it is running in and it returns 
the Pod’s name in the HTTP header making it easy to distinguish it from its other replicas. 
...
First, you will need a containerization software for building containers. Kubernetes 
supports a variety of container types. You will use Docker in this guide. For installation 
instructions, refer to the official Docker documentation.

Next, you will need the Kubernetes command-line tool `kubectl` to interact with your 
Kubernetes cluster. See the https://kubernetes.io/docs/tasks/tools/install-kubectl/#install-kubectl[official documentation^]
for downloading and setting up `kubectl` on your platform.
...
for downloading the latest version of Python.
...
Management (IAM) user. Navigate to the https://console.aws.amazon.com/iam/home#/users[Identity and Access Management^]
...
If you do not have an existing group, you will need to create a new one.
...
The `get-login` command will return a `docker login` command similar to the following.
[source, role="no_copy"]
```
docker login -u AWS -p 123 https://123456.dkr.ecr.[region].amazonaws.com
```
...
```
{
    "repository": {
        ...
        "repositoryArn": "arn:aws:ecr:us-east-2:12345:repository/awsguide/system",
        "createdAt": 1553111916.0,
        "repositoryUri": "12345.ecr.us-east-2.amazonaws.com/awsguide/system"
    }
}
```
...
When tagging and pushing your images, remember to subtitute `[system-repository-uri]` and 
...
your `system` and `inventory` repository URI, respectively.
...
Take note of the `EXTERNAL-IP` in the output of the following command. This will be the 
IP address you substitute into commands later in the guide. 
[role=command]
```
...
Make a request to the `system` microservice substituting in `ip-address` with the `EXTERNAL_IP`
you noted early in this guide.

[source, role="no_copy"]
```
curl http://[ip-address]:31000/system/properties 
```

Make a request to the `inventory` microservice substituting in `ip-address`.

[source, role="no_copy"]
```
curl http://[ip-address]:32000/inventory/systems
```
...
`[ip-address]` with the appropriate value.
...
Since you built a new image, it will have to be pushed to the `awsguide/system` repository
...
----
NAME                                        READY     STATUS    RESTARTS   AGE
system-app-ibm-open-libert-7f78456848-pp46r   1/1       Running   0          6m
inventory-app-ibm-open-libert-6fd6b9dc4-942s2    1/1       Running   0          20m
----

Observe that in this case the `name` microservice is running in the pod called 
`system-app-ibm-open-libert-7f78456848-pp46r`. Substitute the name of your pod into the 
...
Events:
  ...
  ----     ------                 ----  ----                         -------
  Normal   Scheduled              10m   default-scheduler            Successfully assigned system-app-ibm-open-libert-7f78456848-pp46r to docker-for-desktop
  ...
  Normal   Pulled                 10m   kubelet, docker-for-desktop  Container image "system:2.0-SNAPSHOT" already present on machine
