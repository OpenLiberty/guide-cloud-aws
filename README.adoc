...
Re-deploy the `name` microservice by updating the container image to `name:2.0-SNAPSHOT`
in the `kubernetes.yaml` file and running the following command to deploy the resources 
as defined in kubernetes.yaml:
[role='command']
```
kubectl apply -f kubernetes.yaml
```
