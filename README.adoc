...
microservice simply returns the JVM systems properties of the container it is running in 
and it returns the Pod's name in the HTTP header making it easy to distinguish it from 
its other replicas. The `inventory` microservice adds the properties from the `system` 
...
[source, role="no_copy"]
```
curl http://[ip-address]:32000/inventory/systems/system-service
```
...
Events:
  ...
  Normal  Pulling    1m    kubelet, ip.us-east-2.compute.internal  pulling image "1234.dkr.ecr.us-east-2.amazonaws.com/awsguide/system:2.0-SNAPSHOT"
  Normal  Pulled     1m    kubelet, ip.us-east-2.compute.internal  Successfully pulled image "1234.dkr.ecr.us-east-2.amazonaws.com/awsguide/system:2.0-SNAPSHOT"
