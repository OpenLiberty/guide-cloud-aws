...
The `eksctl` CLI tool greatly simplifies the process of creating clusters on EKS. Note that a single `t2.small` Amazon Elastic Compute Cloud (EC2) instance is included in the command by default, this instance is supporting both `i386` and `x86_64` architectures. If you want to build the Docker image that use the other architecture such as ARM64, you can check out the AWS supported instance types by navigating to ##EC2 Dashboard## > ##Instances## > ##Instance Types##. To create your cluster, use the `eksctl create cluster` command:
...
Running this command creates a cluster that is called `guide-cluster` that uses a single `t2.small` instance as the worker node. The `t2.small` EC2 instance is not included in the AWS free tier. See the official https://aws.amazon.com/ec2/pricing/on-demand/[Amazon EC2 pricing^] documentation for more details. When the cluster is created, you see an output similar to the following:
