...
Start by making a repository for the name and ping image, respectively.
[role=command]
```
...
repository URI's as you will need them when tagging and pushign your images.
...
Use the following command to run the integration tests against your cluster. Subtitute `[ip-address]`, `[name-node-port]` and `[ping-node-port]` with the appropriate values.
