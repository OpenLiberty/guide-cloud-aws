...
cluster on Amazon Elastic Container Service for Kubernetes (EKS). You will use `helm` to 
deploy these microservices using Open Liberty Helm charts. Helm is a package manager for 
{kube}. It uses templates to generate {kube} yaml files and then deploys and manages them 
as releases to your cluster.

Amazon Web Services (AWS) offers a managed Kubernetes service called Amazon EKS. EKS simplifies
the process of running Kubernetes on AWS without needing to install or maintain your own
Kubernetes control plane. It provides a hosted {kube} cluster that you may deploy your 
microservices to. You will use it in conjunction with Amazon Elastic Container Registry (ECR).
...
[role=command]
```
aws eks --region [region] update-kubeconfig --name guide-cluster
```
...
10.70.200.73   Ready     <none>    1h        v1.10.11-eks
----
...
Run the following command to verify that the repositories were created. Take note of the
repository URI's as you will need them when tagging and pushing your images.
...
[role=command]
```
docker tag name:1.0-SNAPSHOT [repository-uri]:1.0-SNAPSHOT
docker tag ping:1.0-SNAPSHOT [repository-uri]:1.0-SNAPSHOT
```
...
[role=command]
```
docker push [repository-uri]:1.0-SNAPSHOT
docker push [repository-uri]:1.0-SNAPSHOT
```

When tagging and pushing your images, remeber to subtitute `[repository-uri]` with the 
appropriate URI for the name and ping repositories. 
...
A {kube} deployment is a resource responsible for controlling the creation and management 
of pods. A service exposes your deployment so that you can make requests to your containers. 
Three key items to look at when creating the deployments are the [hotspot=5 hotspot=26 hotspot=34]`label`, 
[hotspot=18 hotspot=39]`image`, and [hotspot=20 hotspot=41]`containerPort` fields. The 
[hotspot=5 hotspot=26 hotspot=34]`label` is a way for a {kube} service to reference specific 
deployments. The [hotspot=18 hotspot=39]`image` is the name and tag of the docker image 
that you want to use for this container. Finally, the [hotspot=20 hotspot=41]`containerPort` 
is the port that your container exposes for purposes of accessing your application.

For the services, the key point to understand is that they expose your deployments. 
The binding between deployments and services is specified by the use of labels -- in this 
case the [hotspot=6 hotspot=10 hotspot=14 hotspot=27 hotspot=31 hotspot=35 hotspot=50 hotspot=64]`app` 
label. You will also notice the service has a type of [hotspot=48 hotspot=62]`NodePort`. 
...
[hotspot=55 hotspot=69]`nodePort` field is not used.
...
[role=command]
```
docker tag name:2.0-SNAPSHOT [repository-uri]:2.0-SNAPSHOT
```
...
[role=command]
```
docker push [repository-uri]:2.0-SNAPSHOT
```
...
Observe that in this case the `name` microservice is running in the pod called `name-app-ibm-open-libert-7f78456848-pp46r`. Substitute the name of your pod into the following command to see more details about the pod.
...
View the events at the bottom of the command's output. Observe that the pod is using the new Docker image `name:2.0-SNAPSHOT`.
...
You have just deployed two microservices to AWS EKS. You have learned to use Helm to install your microservices
